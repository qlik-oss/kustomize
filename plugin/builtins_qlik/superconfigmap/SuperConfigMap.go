...
import (
	...
	qlik "sigs.k8s.io/kustomize/api/builtins_qlik"
...
type SuperConfigMapPlugin struct {
	...
	qlik.SuperMapPluginBase
}

var KustomizePlugin SuperConfigMapPlugin

func (p *SuperConfigMapPlugin) Config(h *resmap.PluginHelpers, c []byte) (err error) {
	p.SuperMapPluginBase = qlik.NewBase(h.ResmapFactory(), p)
